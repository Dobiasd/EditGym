...
import Graphics.Element exposing (Element, flow, outward, sizeOf, container, middle, right, spacer, down, heightOf, empty, leftAligned)
...
referenceBests =
    PersonalBests.readBests referenceBestsJson
...
starsElem below sizeDiv name keyMoves timeSpan =
    let
        elem1 = getKeyMovesStarsElem sizeDiv name keyMoves
...
starsFromPB pbs name =
    case PersonalBests.get pbs name of
        Just pb ->
            ( getKeyMovesStars name pb.keys, getTimeStars name pb.time )
...
starsElemFromPBs below sizeDiv pbs name =
    case PersonalBests.get pbs name of
        Just pb ->
            starsElem False sizeDiv name pb.keys pb.time

        Nothing ->
            let
                elem1 = getStarsWithString sizeDiv "Keys" 0
...
fiveStarsInEverything pbs =
    let
        names = ExercisesList.subjectsWithCat |> map fst

        starPairs = names |> map (starsFromPB pbs)
    in
        all (\p -> p == ( 5, 5 )) starPairs && length pbs >= length names
