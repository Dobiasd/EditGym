...
startButton : Element
startButton =
    niceButton "Start" "?page=game&exercise=introduction"


helpButton : Element
helpButton =
    niceButton "Help" "?page=help"


broseButton : Element
broseButton =
    niceButton "Browse exercises" "?page=exercises"
...
Sometimes you need to edit or rearrange text quickly. Using the mouse is way to slow, but luckily no matter if you use a [word processor](https://www.libreoffice.org/discover/writer), an [editor](http://notepad-plus-plus.org), an [IDE](https://eclipse.org)  or just write an [email](https://www.mozilla.org/en-US/thunderbird/) or a post on [facebook](http://www.facebook.com), the basic keyboard shortcuts are mostly the same.
